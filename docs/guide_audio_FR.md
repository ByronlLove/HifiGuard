...
> Note : le dB(Z) SPL calculé suppose un casque neutre. En réalité, deux autres facteurs modifient le niveau à chaque fréquence : la courbe de réponse du matériel (Hardware Response) et l'EQ logiciel (Peace/APO), si vous en avez un. Les deux s'additionnent : dB(Z) SPL final = MAX_SPL + dBFS + HW(f) + EQ(f)
